...
```{note}
The kernel of the code is based on {cite:t}`gordon1994`.
```

## Thermodynamic derivatives

All thermodynamic first derivatives can be obtained with any set of three independent first derivatives {cite:p}`mcbride1996`. Combustion Toolbox computes all thermodynamic first derivatives from $(\partial \text{ln } V/\partial \text{ln } T)_p$, $(\partial \text{ln } V/\partial \text{ln } p)_T$, and $(\partial h/\partial T)_p = c_p$. Considering the ideal equation of state
