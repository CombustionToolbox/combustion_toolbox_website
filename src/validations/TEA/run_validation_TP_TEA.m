function problems_solved = run_validation_TP_TEA
    % VALIDATION: TP_TEA
    %
    ...
    self = set_prop(self, 'pP', p, 'TP', T); 
    %% SOLVE PROBLEM
    self = SolveProblem(self, 'TP');
    %% SAVE RESULTS
    molar_fractions_CT = cell2vector(self.PS.strP, 'Xi'); 
    problems_solved = length(self.PD.range);
