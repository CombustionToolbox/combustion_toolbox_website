function [N0, dNi_T, dN_T, dNi_p, dN_p, STOP, STOP_ions] = equilibrium_gibbs_eos(self, pP, TP, mix1, guess_moles)
...
%         itMax = 50 + round(S.NS/2);
        ...
        while STOP > TN.tolN && it < itMax
            ...
            Xj = N0(temp_ind, 1) / sum(N0(temp_ind, 1));
            % Estimation of the molar volume (V)
            [V, a_mix, b_mix, ~, a_j, b_j] = eos_PengRobinson(self, S.LS(temp_ind), Xj, TP, pP);
            % Non-ideal contribution of chemical potential
            muRT_ex = mu_ex_eos(self, Xj, TP, pP, V, a_mix, b_mix, a_j, b_j) / R0TP;
            % Chemical potential
            muRT(temp_ind_nswt) =  muRT_0(temp_ind_nswt) + log(N0(temp_ind_nswt, 1) / NP) + log(pP) + muRT_ex;
