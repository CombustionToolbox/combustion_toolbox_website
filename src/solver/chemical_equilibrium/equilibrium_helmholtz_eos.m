...
%     if self.PD.EOS.value
%     dot_ni_ki = dot(N0(temp_ind_nswt, 1), self.PD.covolume(temp_ind_nswt));
%     X = (self.PD.EOS.kappa * dot_ni_ki) / (v * (TP + self.PD.EOS.theta)^self.PD.EOS.alpha);
%     sigma = 1 + X * exp(self.PD.EOS.beta * X);
%     % Activity
%     N = sum(N0(temp_ind_nswt, 1));
%     
%     pP = pP * 1e5; % [Pa]
% 
%     v = v_old - (pP - N * R0TP * df_sigma(self, N0, TP, v_old, temp_ind_nswt))^(-1) * (pP * v_old - N * R0TP * f_sigma(self, N0, TP, v_old, temp_ind_nswt));
%     pP = N * R0TP/v * sigma;
%     STOP_EOS = abs((v - v_old) / v_old);
% 
%     pP = pP * 1e-5; % [bar]
% else
%     return
...
%         itMax = 50 + round(S.NS/2);
        ...
        while STOP > TN.tolN && it < itMax
            it = it + 1;
            % Chemical potential
            muRT_EOS = compute_muRT_EOS(self, TP, vP, N0);
            muRT(temp_ind_nswt) =  muRT_0(temp_ind_nswt) + log(N0(temp_ind_nswt, 1) * R0TP / vP * 1e-5) + muRT_EOS;
...
% SUB-PASS FUNCTIONS
function muRT_EOS = compute_muRT_EOS()
    % Compute non-ideal chemical potential contribution [-]
    if ~self.PD.EOS.value
        muRT_EOS = 0;
    else
        NP = sum(N0(temp_ind, 1));
        dot_xj_kj = dot(N0(temp_ind_nswt, 1) / NP, self.PD.covolume(temp_ind_nswt));
        X = (self.PD.EOS.kappa * dot_ni_ki) / (vP * (TP + self.PD.EOS.theta)^self.PD.EOS.alpha);
        muRT_EOS = exp(self.PD.EOS.beta .* X - 1) ./ self.PD.EOS.beta + self.PD.covolume(temp_ind_nswt) ./ dot_xj_kj .* X .* exp(self.PD.EOS.beta .* X);
    end
end
...
function [A0, ind_A0_E0, NatomE] = remove_elements(NatomE, A0, species, ind_E, guess_moles, tol)
    ...
    if ~isempty(guess_moles)
        pass_ions = guess_moles(temp_ind_ions) > tol;
