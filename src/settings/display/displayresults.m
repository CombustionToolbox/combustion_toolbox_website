...
%         fprintf('EXIT                    Xi [-]\n');
    ...
    fprintf('**************************************************************************************\n\n\n');
elseif nargin == 8
    mix1 = varargin{1}; mix2 = varargin{2}; mix3 = varargin{3}; mix4 = varargin{4}; mix5 = varargin{5};
    fprintf('**************************************************************************************\n');
    fprintf('------------------------------------------------------------------------------------------------------------\n');
    fprintf('Problem type: %s  | phi = %4.4f\n',ProblemType, equivalenceRatio(mix1));
    fprintf('------------------------------------------------------------------------------------------------------------\n');
    if contains(ProblemType, '_OBLIQUE') || contains(ProblemType, '_POLAR_R')
        fprintf('               |     STATE 1     |     STATE 2     |     STATE 3-W   |     STATE 3-S\n');
    elseif contains(ProblemType, '_R')
        fprintf('               |     STATE 1     |     STATE 2     |     STATE 3     |     STATE 3     |     STATE 4\n');
    else
        fprintf('               |  INLET CHAMBER  |     INJECTOR    | OUTLET CHAMBER  |     THROAT      |      EXIT\n');
    end
    
    fprintf('T [K]          |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', temperature(mix1), temperature(mix2), temperature(mix3), temperature(mix4), temperature(mix5));
    fprintf('p [bar]        |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', pressure(mix1), pressure(mix2), pressure(mix3), pressure(mix4), pressure(mix5));
    fprintf('r [kg/m3]      |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', density(mix1), density(mix2), density(mix3), density(mix4), density(mix5));
    fprintf('h [kJ/kg]      |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', enthalpy_mass(mix1), enthalpy_mass(mix2), enthalpy_mass(mix3), enthalpy_mass(mix4), enthalpy_mass(mix5));
    fprintf('e [kJ/kg]      |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', intEnergy_mass(mix1), intEnergy_mass(mix2), intEnergy_mass(mix3), intEnergy_mass(mix4), intEnergy_mass(mix5));
    fprintf('g [kJ/kg]      |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', gibbs_mass(mix1), gibbs_mass(mix2), gibbs_mass(mix3), gibbs_mass(mix4), gibbs_mass(mix5));
    fprintf('s [kJ/(kg-K)]  |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', entropy_mass(mix1), entropy_mass(mix2), entropy_mass(mix3), entropy_mass(mix4), entropy_mass(mix5));
    fprintf('W [g/mol]      |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', meanMolecularWeight(mix1), meanMolecularWeight(mix2), meanMolecularWeight(mix3), meanMolecularWeight(mix4), meanMolecularWeight(mix5));
    fprintf('(dlV/dlp)T [-] |                 |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', mix2.dVdp_T, mix3.dVdp_T, mix4.dVdp_T, mix5.dVdp_T);
    fprintf('(dlV/dlT)p [-] |                 |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', mix2.dVdT_p, mix3.dVdT_p, mix4.dVdT_p, mix5.dVdT_p);
    fprintf('cp [kJ/(kg-K)] |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', cp_mass(mix1), cp_mass(mix2), cp_mass(mix3), cp_mass(mix4), cp_mass(mix5));
    fprintf('gamma [-]      |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', adiabaticIndex(mix1), adiabaticIndex_sound(mix2), adiabaticIndex_sound(mix3), adiabaticIndex_sound(mix4), adiabaticIndex_sound(mix5));
    fprintf('sound vel [m/s]|   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', soundspeed(mix1), soundspeed(mix2), soundspeed(mix3), soundspeed(mix4), soundspeed(mix5));
    fprintf('u [m/s]        |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', velocity_relative(mix1), mix2.v_shock, mix3.v_shock, mix4.v_shock, mix5.v_shock);
    fprintf('Mach number [-]|   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', velocity_relative(mix1)/soundspeed(mix1), mix2.v_shock/soundspeed(mix2), mix3.v_shock/soundspeed(mix3), mix4.v_shock/soundspeed(mix4), mix5.v_shock/soundspeed(mix5));
    if contains(ProblemType, '_OBLIQUE') || contains(ProblemType, '_POLAR')
        fprintf('------------------------------------------------------------------------------------------------------------\n');
        fprintf('PARAMETERS\n');
        fprintf('min wave  [deg]|                 |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', mix2.beta_min, mix3.beta_min, mix4.beta_min, mix5.beta_min);
        fprintf('wave angle[deg]|                 |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', mix2.beta, mix3.beta, mix4.beta, mix5.beta);
        fprintf('deflection[deg]|                 |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', mix2.theta, mix3.theta, mix4.theta, mix5.theta);
        if ~contains(ProblemType, '_OBLIQUE')
            fprintf('max def.  [deg]|                 |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', mix2.theta_max, mix3.theta_max, mix4.theta_max, mix5.theta_max);
            fprintf('sonic def.[deg]|                 |   %12.4f  |   %12.4f  |   %12.4f  |   %12.4f\n', mix2.theta_sonic, mix3.theta_sonic, mix4.theta_sonic, mix5.theta_sonic);
        end
    elseif contains(ProblemType, 'ROCKET')
        fprintf('------------------------------------------------------------------------------------------------------------\n');
        fprintf('PERFORMANCE PARAMETERS\n');
        fprintf('A/At [-]       |                 |                 |   %12.4f  |   %12.4f  |   %12.4f  \n', mix3.Aratio, mix4.Aratio, mix5.Aratio);
        fprintf('CSTAR [m/s]    |                 |                 |   %12.4f  |   %12.4f  |   %12.4f  \n', mix3.cstar, mix4.cstar, mix5.cstar);
        fprintf('CF [-]         |                 |                 |   %12.4f  |   %12.4f  |   %12.4f  \n', mix3.cf, mix4.cf, mix5.cf);
        fprintf('Ivac [s]       |                 |                 |   %12.4f  |   %12.4f  |   %12.4f  \n', mix3.I_vac, mix4.I_vac, mix5.I_vac);
        fprintf('Isp  [s]       |                 |                 |   %12.4f  |   %12.4f  |   %12.4f  \n', mix3.I_sp, mix4.I_sp, mix5.I_sp);
    end
    fprintf('------------------------------------------------------------------------------------------------------------\n');
...
%         fprintf('EXIT                    Xi [-]\n');
    ...
    %%%% SORT SPECIES COMPOSITION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [mix5.Xi(:), ind_sort] = sort(mix5.Xi(:),'descend');
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    j = mix5.Xi>mintol_display;
    Xminor = sum(mix5.Xi(~j));

    for i=1:length(j)
        if j(i)
              fprintf('%-20s %1.4e\n', ListSpecies{ind_sort(i)}, mix5.Xi(i));
        end
    end
    Nminor = length(mix5.Xi) - sum(j);
    s_space_Nminor = char(32 * ones(1, 4 - numel(num2str(Nminor))));
    fprintf('MINORS[+%d] %s     %12.4e\n\n', Nminor, s_space_Nminor, Xminor);
    fprintf('TOTAL            %14.4e\n', sum(mix5.Xi));
    fprintf('------------------------------------------------------------------------------------------------------------\n');
